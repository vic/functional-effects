...
object ZIOModel {
  ...
  final case class ZIO[-R, +E, +A](run: R => Either[E, A]) { self =>
    ...
    def flatMap[R1 <: R, E1 >: E, B](f: A => ZIO[R1, E1, B]): ZIO[R1, E1, B] =
      ZIO[R1, E1, B](r1 => run(r1).flatMap(a => f(a).run(r1)))

    def zip[R1 <: R, E1 >: E, B](that: ZIO[R1, E1, B]): ZIO[R1, E1, (A, B)] =
      ZIO[R1, E1, (A, B)] { r =>
        self.run(r).flatMap(a => that.run(r).map(a -> _))
      }
...
object ZIOTypes {
  ...
  type UIO[+A]      = ZIO[Any, Nothing, A] // Seq(1, 2, 3).min => 1 // Seq.empty[Int].min // *Throwable
...
object SimpleMap extends App {
  ...
  val readLine: URIO[Console, String] = getStrLn.orDie
...
object PrintSequenceZip extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] = {
    val zios = 0.to(10).map(n => putStrLn(n.toString))
    val all = zios.fold(ZIO.unit) { case (a, b) => a.zipRight(b).unit }
    all.as(ExitCode.success)
  }
}
...
object PrintSequence extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    (putStrLn("a") *> putStrLn("b") *> putStrLn("c")).as(ExitCode.success)
}
...
object PrintReadSequence extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    putStrLn("Presiona enter para salir") *> readLine *> ZIO.succeed(ExitCode.success)
}
...
object PromptName extends App {
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    putStrLn("What is your name?").flatMap { _ =>
      readLine.flatMap(name => putStrLn(s"Your name is: ${name}")).flatMap { _ =>
        ZIO.succeed(ExitCode.success)
      }
    }
...
object ForComprehension extends App {
  ...
  def printName(x: String) = putStrLn(s"Your name is ${x}")
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] = {
    putStrLn("Your name") *> (readLine >>= printName).exitCode
  }
  //
//    for {
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
      putStrLn("How old are you?").flatMap { _ =>
        readInt.flatMap { age =>
          ...
          else putStrLn("You are all grown up!")).map { _ =>
            ExitCode.success
          }
        }
      }
